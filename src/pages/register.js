...
export default function Register(){
    ...
    /*const handleGoogleLogin = async () => {
        ...
        try {
            ...
            // شيك لو المستخدم جديد وسجله في Firestore
    ...
    return(
        <>
        <Head>
            <title>Register - Chickana dd</title>
            <meta name="description" content="chickana register" />
        </Head>
        <main>
            <div className="bg-gray-100 p-[30px] sm:p-[80px]">
                <form onSubmit={handleSubmit} className="max-w-[400px] sm:w-[400px] m-auto bg-[#fff] px-[25px] py-[15px] flex flex-col gap-[15px] rounded-[5px] shadow-sm">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-[10px] gap-y-[15px]">
                        <input onChange={handleChange} value={form.pass} type="password" name="pass" placeholder="Password" autoComplete="new password" className="p-[10px] border rounded-[5px] w-full" />
                        <input onChange={handleChange} value={form.passConfirm} type="password" name="passConfirm" placeholder="Password Confirm" autoComplete="new password" className="p-[10px] border rounded-[5px] w-full" />
                        ...
                        <input onChange={handleChange} value={form.fName} type="text" name="fName" placeholder="First Name" autoComplete="user name" className="p-[10px] border rounded-[5px] w-full" />
                        <input onChange={handleChange} value={form.lName} type="text" name="lName" placeholder="Last Name" autoComplete="user name" className="p-[10px] border rounded-[5px] w-full" />
                    </div>
                    ...
                    <Link href="/login" className="hover:underline transition duration-300 text-center text-[14px]">Login</Link>
                </form>
            </div>
        </main>
        </>
